''' 
This file contains the data manipulation and data visualization w.r.t police deaths of K9 Unit.
...
def k9_unit_analysis(df, user_menu, unit_menu):
    '''
    ...
    Warning :-
    This function is especially made for the dataframe which can be acquired from "k9_deaths.csv".
    It will not work on any other dataframes.

    The k9_deaths df contains this columns ->
    ['Rank', 'Name', 'Breed', 'Gender', 'Age', 'End of watch', 'Day of week', 'Cause', 'Department', 'State', 'Tour', 'Weapon', 'Offender', 'Summary', 'Year', 'Month', 'Day'].
    ...
    if user_menu == "Overall Analysis":
        ...
        types_of_breeds = df['Breed'].unique().tolist()
        types_of_breeds.remove(np.nan)  
        unique_breeds = len(types_of_breeds) # Number of Unique types of Breeds.

        breed_with_most_death = df['Breed'].value_counts().sort_values(ascending=False).reset_index()
        breed_with_most_death = breed_with_most_death.iloc[0][0] # Highest number of deaths by breed
        ...
        with col2:
            st.subheader("Unique Breeds")
            st.header(unique_breeds) 
        ...
        with col2:
            st.subheader("Breed With Most Deaths")
            st.header(breed_with_most_death) 
        ...
        age_distribution_fig = helper.age_distribution("preprocessed data/k9_unit_age_distribution_df.csv")
        st.plotly_chart(age_distribution_fig)  

        heatmap_fig = helper.heatmap_year_vs_cause("preprocessed data/k9_unit_heatmap_year_vs_cause.csv")
    ...
    if user_menu == "Top Ten Deadly...":
        ...
        # Presents a list to select a filter [Rank, Breed, State, etc.].
    ...
    if user_menu == "State-Wise Analysis":
        ...
        # Presents a list to select of States (default -> "California").
        selected_state = st.selectbox("Select a State", state_selection_list, index=state_selection_list.index("California"))
        ...
        state_heatmap_fig = helper.heatmap_year_vs_cause("preprocessed data/k9_unit_heatmap_year_vs_cause.csv", selected_state)
    ...
    if user_menu == "Cartogram":
        st.title("K9 Deaths in USA Cartogram")
        ...
        map_fig = helper.cartogram("preprocessed data/k9_unit_filtered_states_for_map.csv", year_range)
